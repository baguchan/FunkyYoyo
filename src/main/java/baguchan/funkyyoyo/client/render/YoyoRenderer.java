...
public class YoyoRenderer extends EntityRenderer<Yoyo> {
    ...
    public void render(Yoyo entity, float p_116112_, float partialTicks, PoseStack stackIn, MultiBufferSource p_116115_, int p_116116_) {
        ...
        if (entity.canAttach()) {
            stackIn.mulPose(Axis.XP.rotationDegrees((float) ((entity.getSpeed() * 360F))));
        } else {
            stackIn.mulPose(Axis.XP.rotationDegrees((float) ((entity.tickCount + partialTicks) * 40F + (entity.getSpeed() * 360F))));
        }
